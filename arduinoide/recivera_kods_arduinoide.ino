...
bool status=0; //ieslegts/izslegts
...
void loop() { //galvena dala, kas nodrosina recivera inicializesanu, rezimu parsleksanu, pirmo rezimu un MUTE funkcionalitati
  ...
  while(status==0){ //parbauda vai ierica ir izslegta, veic stavoklim atbilstosas darbibas
    if(pfcikls==1){
      status=1;
    ...
    if(pogaval==1){
      ...
      //norada ka recivers ir ieslegts, veic stavoklim atbilstosas darbibas
      status=1;
      stmaina=1;
